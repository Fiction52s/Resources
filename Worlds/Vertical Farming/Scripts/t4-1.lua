...
function bosscam( entered, pos, halfWidth, halfHeight)
	if entered then
...
function UpdatePostPhysics()
	...
	if bossCam then
		if camx < left + 14 * zoom then camx = left + 14 * zoom end
		if camx > right - 14 * zoom then camx = right - 14 * zoom end
		if camy < top + 7.5 * zoom then camy = top + 7.5 * zoom end
		if camy > bottom - 7.5 * zoom then camy = bottom - 7.5 * zoom end
	end
	
	stage:SetCameraPosition( camx, camy )
	
	
	--if stage.a_elevator1:GetVelocity().x ~= 0 or stage.a_elevator1:GetVelocity().y ~= 0 then
	--	stage:SetCameraPosition( stage.a_elevator1:GetPosition().x, stage.a_elevator1:GetPosition().y )
	--end
