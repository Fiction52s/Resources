...
--[[zoomGoal = 1
	...
	zoomGoal = 1 + ( math.abs(player:GetVelocity().x - player:GetCarryVelocity().x) * .02)
	...
	if math.abs( player:GetVelocity().x - player:GetCarryVelocity().x ) > 17 then
		zoomedOut = true
		slowCounter = 0
	...
	--old.x = player:GetVelocity().x
	--old.y = player:GetVelocity().y--]]
end
