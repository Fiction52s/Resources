...
--	lobber1Set = actor:TileSetIndex( "lobber1.png" )
	springSet = actor:TileSetIndex( "weakspring.png" )
	...
	--if action == throw then
	  ...
	  if throwActor and action ~= throw then
		--if frame == 1 then
			...
			local x = --thrownActor:GetVelocity().x * math.cos(angle) + thrownActor:GetVelocity().y * math.sin( angle ) + 30 * math.sin(angle)
			30 * math.sin(angle) + thrownActor:GetVelocity().x * math.cos( angle )
			local y = --thrownActor:GetVelocity().x * math.sin(angle) + thrownActor:GetVelocity().y * math.cos( angle ) - 30 * math.cos( angle )
			-30 * math.cos(angle) --+ thrownActor:GetVelocity().y * math.sin( angle )
			--thrownActor:SetVelocity( thrownActor:GetVelocity().x, -30 )
			local s = math.sin( angle )
			local c = math.cos( angle )
			
			
			--if s >= 0 then
			if 30 * s > 0 and thrownActor:GetVelocity().x * c > 0 or 30 * s < 0 and thrownActor:GetVelocity().x * c < 0 then
				print( "limiting x from " .. x .. " to " .. 30 * s )
				--x = 30 * s
				
			end
			
			if -30 * c > 0 and thrownActor:GetVelocity().y * s > 0 or -30 * c < 0 and thrownActor:GetVelocity().y * s < 0 then
				y = -30 * c
			end
