...
function Init()
	...
	wormSet = actor:TileSetIndex( "worm.png" )
	...
	--actor:SetBodyAngle( .125 * math.pi )
	actor:SetBodyAngle( 0 )
	
	jumpPrepare = {}
	for i = 1, 40 do
		if i < 20 then
			jumpPrepare[i] = {wormSet, 1 }
		else
			jumpPrepare[i] = {wormSet, 2 }
		end
	end
	...
	actor:CreateCircle( 12, Layer_PlayerDetection, 0, 0, 4 )
	actor:CreateBox( bodyTypes.Normal, Layer_EnemyPhysicsbox, 0, 0, .4, 2, 0 )
	actor:CreateBox( bodyTypes.Normal, Layer_EnemyHitbox, 0, 0, .4, 2, 0 )
	actor:CreateBox( bodyTypes.Normal, Layer_EnemyHurtbox, 0, 0, .4, 2, 0 )
	triggeredActivation = false
	actor:SetFriction( 0 )
...
function ActionEnded()
	if frame > #action then 
		if action == jump then
			frame = 3 
		elseif action == jumpPrepare then
			SetAction( jump )
			frame = 1
...
function ChooseAction()
	if triggeredActivation and action == exist then
		--triggeredActivation = false
		SetAction( jumpPrepare )
		frame = 1
		print( "jumpprep" )
	end
	...
	if grounded and not lastGrounded then 
		...
		frame = 1
		actor:SetVelocity( 0, 0 )
		actor:SetBodyAngle( math.atan2( groundNormal.x, -groundNormal.y ) )
		print( "exist: " .. actor:GetBodyAngle()  )
	end
	--print( "srsly here guys" )
end

function HandleAction()
	if grounded and action == jump and frame == 3 then
		grounded = false
		--actor:SetVelocity( math.sin( angle ) * 10, math.cos( angle ) * 10 )
		
		local a = actor:GetBodyAngle() --/ math.pi
		...
		if groundNormal.y < 0 then
			if player:GetPosition().x < actor:GetPosition().x then
				...
				if player:GetPosition().y < actor:GetPosition().y then
					actor:SetVelocity( -jumpStrengthX, -jumpStrengthY - 5 )--- 5 )
				else
					actor:SetVelocity( -jumpStrengthX, -jumpStrengthY )
				end
				
				--actor:SetVelocity( -jumpStrengthX * math.cos( a ) + jumpStrengthY * math.sin( a ), -jumpStrengthX * math.cos( a ) + (-jumpStrengthY) * math.cos( a ) )
			else
				if player:GetPosition().y < actor:GetPosition().y then
					actor:SetVelocity( jumpStrengthX, -jumpStrengthY - 5 )--- 5 )
				else
			...
			actor:SetVelocity( actor:GetVelocity().x, actor:GetVelocity().y - math.random(-3,3) )
		elseif groundNormal.y > 0 then
			if player:GetPosition().x < actor:GetPosition().x then
				
				if player:GetPosition().y < actor:GetPosition().y then
					actor:SetVelocity( -jumpStrengthX, jumpStrengthX / 2 )
				else
					actor:SetVelocity( -jumpStrengthX, jumpStrengthX / 2 )
				end
			...
			actor:SetVelocity( 10 * math.sin( a ), -10 )
		end
		
		print( "set vel: " .. actor:GetVelocity().x .. ", " .. actor:GetVelocity().y )
		print( "jump A: " .. a )
		justJumping = true
		
	end
	
	if action == jump and frame == 3 then
		actor:SetBodyAngle( math.atan2( actor:GetVelocity().x, -actor:GetVelocity().y )  )
		print( "setting body angle: " .. math.atan2( actor:GetVelocity().x, -actor:GetVelocity().y )  )
	end
end
...
--		actor:FaceLeft()
	...
	actor:SetSpriteAngle( 0, actor:GetBodyAngle() / math.pi ) 
	lastGrounded = grounded
end
...
function HandleStageCollision( pointCount, point1, point2, normal, enabled )
	...
	if enabled and action == jump and frame == 3 and not justJumping then
		groundNormal.x = normal.x
		groundNormal.y = normal.y
		grounded = true
		return true
